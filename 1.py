...
# 不平衡问题处理
# SMOTE法过采样
# smote = SMOTE(random_state=0,k_neighbors=2,n_jobs=1)  # random_state为0（此数字没有特殊含义，可以换成其他数字）使得每次代码运行的结果保持一致
# X_smo, y_smo = smote.fit_resample(X_train, y_train.astype('int'))  # 使用原始数据的特征变量和目标变量生成过采样数据集
# print('SMOTE过采样后标签数据统计：', Counter(y_train))
...
#     'max_depth':[5, 6, 7, 8, 9, 10, 11, 12],   # 深度：这里是森林中每棵决策树的深度
...
#选取4种分类算法
clf = RandomForestRegressor(n_estimators=100,random_state=80, min_samples_split=2,max_features=0.5,max_depth=7)
