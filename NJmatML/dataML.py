...
def xgboost_default():
    ...
    clf_xgboost_default = XGBRegressor()
...
def RandomForest_default():
    ...
    global clf_rf
    clf_rf = ensemble.RandomForestRegressor(n_estimators=100,random_state=80, min_samples_split=2,max_features=0.5,max_depth=7)
    ...
    from sklearn.model_selection import cross_val_score
    kfold = KFold(n_splits=10)
    scores = cross_val_score(clf_rf, X_train, y_train, scoring='r2', cv=kfold)
    # scoring='neg_mean_squared_error'
    print("scores:", scores)
    scores_fold = []
    for i in range(len(scores)):
    ...
    plt.show()
    # 训练集也可以打印准确率并plot图
    y_train_prediction = clf_rf.predict(X_train)
    mse_train = mean_squared_error(y_train, y_train_prediction)
    rmse_train = mse_train ** (1/2)
    from sklearn.metrics import mean_absolute_error
    MAE_train = mean_absolute_error(y_train, y_train_prediction)
...
def xgboost_default_predict(csvName):
    ...
    writer = pd.ExcelWriter('Predict_xgboost_dataset.xlsx')
    predict_Ef.to_excel(writer, 'Sheet1')
    writer.save()

# 7.1.1.2 预测集基于random_forest_default()
def randomforest_default_predict(csvName):
    import pandas as pd
    Predict_features = pd.DataFrame(pd.read_csv(csvName))
    featureData1 = Predict_features.values[:, :]
    # StandardScaler.fit(featureData1)
    # featureData2 = StandardScaler.transform(featureData1)
    # print(featureData2)
    predict = clf_rf.predict(featureData1)
    predict_Ef = pd.DataFrame(predict)
    writer = predict_Ef.to_csv('Predict_rf_dataset.csv')
